...
public class PasswordEvaluator {
	...
	public static String evaluatePassword(String input) {
		// The following are the local variable used to perform the FSM
		passwordErrorMessage = "";
		passwordIndexofError = 0;			// Initialize the IndexofError
		inputLine = input;					// Save the reference to the input line as a global
		currentCharNdx = 0;					// The index of the current character
		
		if(input.length() <= 0) return "\nWarning: The entered password field is empty";
		
		// The input is not empty, so we can access the first character
		
		// Set up flags to check for the password requirements
		...
		// The FSM simulation continues until the end of the input is reached or at some 
		// state the current character does not match any valid transition
		while (running) {
			displayInputState();
			// The cascading if statement sequentially tries the current character against all of the
			// valid transitions until end of input has been reached or an invalid char is detected
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				// Handles any other special character that has not been specified.
		...
		passwordIndexofError = currentCharNdx;
		return "Warning, password must have the following: " + errMessage.substring(0, errMessage.length() - 2);
