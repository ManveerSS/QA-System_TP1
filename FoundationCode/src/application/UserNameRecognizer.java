...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		if(input.length() <= 0) {
			...
			return "\nWarning: The entered username field is empty";
		}
		...
		while (running) {
			...
			case 0: 
				...
				// The first character is checked against A-Z or a-z. If any are matched
			...
			case 1: 
				...
				//	1: a A-Z, a-z, 0-9 that self-loops back to state 1
				//  2: either a period, underscore, or minus sign that transitions to state 2 
			...
			case 2: 
				// State 2 deals with a character after a ., -, or _ in the username.
				
				// The if-statement ensures that the next character must be alphanumeric
				// A-Z, a-z, 0-9 -> State 1
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) ||	// Check for a-z
						(currentChar >= '0' && currentChar <= '9' )) {	// Check for 0-9
		...
		case 1:
			// State 1 is a final state.  Check to see if the UserName length is valid.
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"Invalid special character deteted.\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
